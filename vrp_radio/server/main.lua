...
AddEventHandler('vrp_radio:isCop', function(user)
	...
	if (vRP.hasGroup({user_id,"cop"}) and vRP.tryGetInventoryItem({user_id,"radio",1, false})) or (vRP.hasPermission({user_id, "radio.police"}) and vRP.tryGetInventoryItem({user_id,"radio",1, false})) then
		vRP.giveInventoryItem({user_id,"radio",1, false})
