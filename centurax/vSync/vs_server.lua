...
vRPclient = Tunnel.getInterface("vRP","vSync")
...
function isAllowedToChange(player)
    ...
    local allowed = false
    if vRP.hasGroup({user_id,"Fondator"}) or vRP.hasGroup({user_id,"scripter"}) or vRP.hasGroup({user_id,"admin"}) or vRP.hasGroup({user_id,"moderator"}) or vRP.hasGroup({user_id,"helper"}) then   
        allowed = true
    end
    return allowed
end
...
RegisterCommand('freezetime', function(source, args)
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            if freezeTime then
                vRPclient.notify(source,{"Vremea este blocata"})
            else
                vRPclient.notify(source,{"Vremea nu mai este blocata"})
            end
        else
    ...
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            if not DynamicWeather then
                vRPclient.notify(source,{"Modificările meteorologice dinamice sunt acum dezactivate."})
                --TriggerClientEvent('vSync:notify', source, 'Modificările meteorologice dinamice sunt acum dezactivate.')
            else
                vRPclient.notify(source,{"Modificările meteorologice dinamice sunt acum activate."})
